...
function toScore(v) {
    ...
    if (n > 0 && n <= 1) n *= 100; // fractions: 0.85 -> 85
    n = Math.round(n);
    if (n < 0 || n > 100) return null; // excludes garbage like "8500%"
    return n;
}
...
function renderGroupedBarChart(data) {
    ...
    data.forEach(r => {
        ...
        const push = (arr, val) => { const s = toScore(val); if (s !== null) arr.push(s); };
        push(groups[lob].reliable,   r['RELIABLE']);
...
function renderSupervisorDashboard(data) {
    ...
    const passed = data.filter(rowIsPassed).length;
