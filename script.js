...
const NON_ISSUE_VALUES = new Set([
    '', 'NO OPPORTUNITY', 'NA', 'N/A', 'NO', 'NONE', 'NO OPPORTUNITY'
]);
...
function isNA(v) {
    const n = normVal(v);
    return n === '' || n === 'NA' || n === 'N/A' || n === 'NONE';
}

function isNoOpportunity(v) {
    return normVal(v) === 'NO OPPORTUNITY';
}

function scoreDescriptiveParam(value) {
    if (isNA(value)) return null; // excluded
    return isNoOpportunity(value) ? 1 : 0;
}

function scoreBooleanParam(value, hitValue) {
    if (isNA(value)) return null; // excluded
    const n = normVal(value);
    if (n === normVal(hitValue)) return 0; // hit = bad
    if (n === 'YES' || n === 'NO OPPORTUNITY') return 1; // good
    return 0; // any other descriptive text = issue found
}

function calculateCategoryScore(scores) {
    const valid = scores.filter(s => s !== null);
    if (valid.length === 0) return 1; // default to 100% if all NA
    return valid.reduce((a, b) => a + b, 0) / valid.length;
}

function calculateRowScores(row) {
    // MISTREAT
    const mistreatVal = normVal(row['IS THIS A POTENTIAL CUSTOMER MISTREAT?']);
    const mistreatScore = mistreatVal === 'NO' ? 1 : (mistreatVal === 'YES' ? 0 : 1);

    // RELIABLE (4 params)
    const reliableParams = [
        row['IRRELEVANT SOLUTION'],
        row['INCOMPLETE SOLUTION'],
        row['UNTIMELY SOLUTION ( ZTP)'],
        row['UNCLEAR SOLUTION']
    ];
    const reliableScores = reliableParams.map(v => scoreDescriptiveParam(v));
    const reliableScore = calculateCategoryScore(reliableScores);

    // PERSONABLE (7 params)
    const personableParams = [
        row['Poor Listening Skills?'],
        row['Customer Validation and Empathy Gap?'],
        row['Did not adjust the tone/pace to match the customer?'],
        row['Did not adjust to the customers language?'],
        row['Negative Words, Phrasing and Limitations?'],
        row['Unfriendly/discourteous/sarcastic?'],
        row['Sounded transactional or robotic?']
    ];
    const personableScores = personableParams.map(v => scoreDescriptiveParam(v));
    const personableScore = calculateCategoryScore(personableScores);

    // FAST (1 param)
    const fastVal = row['FAST: Were there other Agent factors observed that affected the customer experience?'];
    const fastScore = isNA(fastVal) ? 1 : (isNoOpportunity(fastVal) ? 1 : 0);

    // SAFE & SECURE (9 params)
    const safeParams = [
        { val: row['DID WE FOLLOW THE CUSTOMER AUTHENTICATION PROCESS?'], hit: 'NO' },
        { val: row['DID WE FOLLOW THE DATA PRIVACY POLICY?'], hit: 'NO' },
        { val: row['DID WE UPDATE THE CUSTOMER INFORMATION IN THE TOOL?'], hit: 'NO' },
        { val: row['DID WE FOLLOW THE CSAT/NPS PROCESS?'], hit: 'NO' },
        { val: row['DID WE FOLLOW THE SYSTEM DOCUMENTATION PROCESS?'], hit: 'NO' },
        { val: row['DID WE FOLLOW THE SYSTEM TAGGING PROCESS?'], hit: 'NO' },
        { val: row['DID WE FOLLOW CORRECT GRAMMAR, TECHNICAL WRITING & THE PRESCRIBED LANGUAGE?'], hit: 'NO' },
        { val: row['DID THE AGENT OFFER SELF-CARE HELP TO THE CUSTOMER?'], hit: 'NO' },
        { val: row['DID THE AGENT UPSELL OR CROSS SELL RELEVANT PRODUCTS & SERVICES'], hit: 'NO' }
    ];
    const safeScores = safeParams.map(p => scoreBooleanParam(p.val, p.hit));
    const safeScore = calculateCategoryScore(safeScores);

    // OVERALL SCORE (Weighted)
    const overallScore = (
        reliableScore * SCORING_WEIGHTS.RELIABLE +
        personableScore * SCORING_WEIGHTS.PERSONABLE +
        fastScore * SCORING_WEIGHTS.FAST +
        safeScore * SCORING_WEIGHTS.SAFE_SECURE
    );

    // CLUSTER ASSIGNMENT
    let cluster = 'D';
    if (overallScore >= 0.90) cluster = 'A';
    else if (overallScore >= 0.80) cluster = 'B';
    else if (overallScore >= 0.70) cluster = 'C';

    return {
        mistreat: mistreatScore,
        reliable: reliableScore,
        reliableParams: reliableScores,
        personable: personableScore,
        personableParams: personableScores,
        fast: fastScore,
        safeSecure: safeScore,
        safeParams: safeScores,
...
async function quickAccess(role) {
    ...
    try {
        ...
        if (!profileSnap.exists()) {
            return showAuthMsg('loginMsg', 'No user profile document found in Firestore.', false);
        }
...
async function enterApp() {
    ...
    if (sessionLabel) sessionLabel.textContent = (roleLabels[normalizedRole] || '👤 ') + currentSession.email;
    ...
    if (canViewDashboard) {
        ...
        if (dataStatus) {
            dataStatus.innerHTML = rows.length
                ...
                : `⚠️ 0 rows. Use upload button to import data.`;
        }
...
async function handleRosterUpload(event) {
    ...
    try {
        ...
        const emailKey = findHeader(rows[0], ['PLDT/SMART Domain v2', 'PLDT/SMART Domain', 'Domain', 'Email', 'Work Email']);
        ...
        if (!emailKey || !nameKey) throw new Error('Missing required Roster headers.');
        const roster = rows
            .map(r => ({
...
async function refreshRosterStatus() {
    ...
    } catch (err) { console.warn("Roster status read failed:", err); }
}
...
async function resyncAgentEmails() {
    ...
    try {
        ...
        await Promise.all(promises);
        if (statusEl) statusEl.textContent = `✅ Re-synced: ${matched} matched, ${unmatched} unmatched.`;
    } catch (err) {
        console.error(err);
...
async function handleDataUpload(event) {
    ...
    try {
        ...
        const processed = rows.map(r => {
            ...
            // Calculate scores
            const scores = calculateRowScores(out);
            out['MISTREAT'] = scores.mistreat;
            out['RELIABLE'] = Math.round(scores.reliable * 100);
            out['PERSONABLE'] = Math.round(scores.personable * 100);
            out['FAST'] = Math.round(scores.fast * 100);
            out['SAFE & SECURE'] = Math.round(scores.safeSecure * 100);
            out['OVERALL SCORE'] = Math.round(scores.overall * 100);
            out['CLUSTER'] = scores.cluster;
...
function filterData() {
    ...
    if (!rows || !rows.length) { renderSupervisorDashboard([]); return; }
    const getValue = (id) => { const el = document.getElementById(id); return el ? el.value : 'ALL'; };
    const f = {
...
function renderGroupedBarChart(data) {
    ...
    lobChartInstance = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                legend: { position: 'top', labels: { usePointStyle: true, pointStyle: 'rect', padding: 15, font: { size: 10, weight: 'bold' } } },
                datalabels: { anchor: 'end', align: 'top', offset: 2, formatter: (val) => val ? val + '%' : '', font: { size: 8, weight: 'bold' }, color: '#333' }
            },
            scales: {
                y: { display: false, max: 115 },
                x: { grid: { display: false }, ticks: { font: { size: 9, weight: '600' }, color: '#333', maxRotation: 45, minRotation: 0, autoSkip: false } }
            }
        }
    });
}
...
function renderSupervisorDashboard(data) {
    ...
    if (!data || !data.length) {
        ...
        if (topHitsBody) topHitsBody.innerHTML = '<tr><td colspan="3" class="empty-note">No matching audit data.</td></tr>';
    ...
    const avgOverall = (() => {
        const vals = data.map(r => r['OVERALL SCORE']).filter(v => v !== null && !isNaN(v));
    ...
    data.forEach(r => {
        ...
        if (r['OVERALL SCORE'] !== null && !isNaN(r['OVERALL SCORE'])) {
    ...
    if (topHitsBody) {
        topHitsBody.innerHTML = sortedHits.length
            ...
            : '<tr><td colspan="3" class="empty-note">No parameters flagged.</td></tr>';
    }
}
...
async function renderAgentView() {
    ...
    if (welcomeName) welcomeName.textContent = 'Welcome, ' + (currentSession.agentName || currentSession.email);
    let myRows = [];
    try {
    ...
    } catch (err) { console.error("Agent query failed:", err); }
    ...
    const auditRowHtml = (r) => {
        ...
        const tagsHtml = issues.length
            ...
            : `<span class="no-issues-note">✓ No parameters flagged.</span>`;
        ...
        return `<div class="audit-row">
            ...
            <div class="audit-meta">TL: ${escapeHtml(r['TEAM LEADER']) || '—'} · Cluster: ${escapeHtml(r['CLUSTER']) || '—'} · Month: ${escapeHtml(r['MONTH']) || '—'}</div>
            <div>${tagsHtml}</div>
            ${commentsHtml}
        </div>`;
    };
    ...
    if (agentAuditList) {
        agentAuditList.innerHTML = orderedMonths.map((month, idx) => {
            ...
            const monthAvg = (() => {
                const vals = rows.map(r => r['OVERALL SCORE']).filter(v => v !== null && !isNaN(v));
                return vals.length ? Math.round(vals.reduce((a, b) => a + b, 0) / vals.length) : null;
            })();
...
/* ==========================================================================
   EXCEL SCORE GENERATOR (Export Function)
   ========================================================================== */
function generateScoredExcel() {
    if (!cachedAuditRows.length) {
        alert('No data loaded. Please upload data first.');
        return;
    }

    const scoredRows = cachedAuditRows.map(row => {
        const scores = calculateRowScores(row);
        return {
            'ID': row['ID'],
            'AGENT/OFFICER NAME': row['AGENT/OFFICER NAME'],
            'FORM TYPE': row['FORM TYPE'],
            'LINE OF BUSINESS': row['LINE OF BUSINESS'] || row['BRAND'],
            'WEEKENDING': row['WEEKENDING'],
            'MONTH': row['MONTH'],
            'AGENT TENURE': row['AGENT TENURE'],
            'TEAM LEADER': row['TEAM LEADER'],
            'IS THIS A POTENTIAL CUSTOMER MISTREAT?': row['IS THIS A POTENTIAL CUSTOMER MISTREAT?'],
            'MISTREAT SCORE': scores.mistreat === 1 ? '100%' : '0%',
            'IRRELEVANT SOLUTION': row['IRRELEVANT SOLUTION'],
            'IRRELEVANT SCORE': scores.reliableParams[0] === null ? 'NA' : scores.reliableParams[0],
            'INCOMPLETE SOLUTION': row['INCOMPLETE SOLUTION'],
            'INCOMPLETE SCORE': scores.reliableParams[1] === null ? 'NA' : scores.reliableParams[1],
            'UNTIMELY SOLUTION (ZTP)': row['UNTIMELY SOLUTION ( ZTP)'],
            'UNTIMELY SCORE': scores.reliableParams[2] === null ? 'NA' : scores.reliableParams[2],
            'UNCLEAR SOLUTION': row['UNCLEAR SOLUTION'],
            'UNCLEAR SCORE': scores.reliableParams[3] === null ? 'NA' : scores.reliableParams[3],
            'RELIABLE SCORE': Math.round(scores.reliable * 100) + '%',
            'PERSONABLE SCORE': Math.round(scores.personable * 100) + '%',
            'FAST SCORE': Math.round(scores.fast * 100) + '%',
            'SAFE & SECURE SCORE': Math.round(scores.safeSecure * 100) + '%',
            'OVERALL SCORE': Math.round(scores.overall * 100) + '%',
            'CLUSTER': scores.cluster,
            'OVERALL PASSRATE': scores.overall >= 0.85 ? 'PASSED' : 'FAILED',
            'CM': scores.overall >= 0.95 ? 'SUPERSTAR' : (scores.overall < 0.70 ? 'UNDERPERFORMER' : 'AVERAGE')
        };
    });

    const ws = XLSX.utils.json_to_sheet(scoredRows);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'SCORED DATA');

    // Summary Sheet
    const summaryData = [{
        'Total Audits': scoredRows.length,
        'Pass Rate': Math.round(scoredRows.filter(r => r['OVERALL PASSRATE'] === 'PASSED').length / scoredRows.length * 100) + '%',
        'Average Overall Score': Math.round(scoredRows.reduce((a, r) => a + parseInt(r['OVERALL SCORE']), 0) / scoredRows.length) + '%',
        'Cluster A Count': scoredRows.filter(r => r['CLUSTER'] === 'A').length,
        'Cluster B Count': scoredRows.filter(r => r['CLUSTER'] === 'B').length,
        'Cluster C Count': scoredRows.filter(r => r['CLUSTER'] === 'C').length,
        'Cluster D Count': scoredRows.filter(r => r['CLUSTER'] === 'D').length,
        'Generated Date': new Date().toISOString().split('T')[0]
    }];
    const ws2 = XLSX.utils.json_to_sheet(summaryData);
    XLSX.utils.book_append_sheet(wb, ws2, 'SUMMARY');

    XLSX.writeFile(wb, `PLDT_Quality_Scores_Generated_${new Date().toISOString().split('T')[0]}.xlsx`);
}
...
window.generateScoredExcel = generateScoredExcel;
